...
/* 	GÖREV 3:
	- Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş tam sayılar ve 
  ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
    ...
    yazılmış sayıları tespit ederek kaç kere tekrar edildiğini belirten bir string oluşturulup `tekraredensayilar` dizisine aktarılmasını istiyoruz. Örnek string: "{sayı} sayısı {tekrarSayisi} kere tekrar edilmiştir"
...
enbuyuk = sayilar[0];
enkucuk =sayilar[0];
for(let i = 0 ; i < sayilar.length ; i += 1){
...
ucebolunenlerintoplami = ucetambolunenler.reduce((toplam,rakam) => (toplam + rakam),0);

//3d. `besyuzdenkucuksayilar` adında bir dizi oluşturarak, sayilar dizisinin içindeki 500'den küçük sayıları bu diziye atayın (.filter metodunu kullanın)
// 3d çözümü
besyuzdenkucuksayilar = [];
sayilar.filter((sayi) => {if(sayi < 500){
...
/* 3f. `tekraredensayilar` adında bir dizi oluşturun. sayilar dizisi içerisindeki bazı sayılar birden fazla kere yazılmış. sayilar dizisi içerisinde birden fazla kez 
    yazılmış sayıları tespit ederek kaç kere tekrar edildiğini belirten bir string oluşturulup `tekraredensayilar` dizisine aktarılmasını istiyoruz. 
...
for(const sayi of sayilar){
  let toplamRakam = 0;
  for(const rakam of sayilar){
    if(sayi === rakam){
      toplamRakam += 1; 
  }
}
  tekraredensayilar.push(`${sayi} sayısı ${toplamRakam} kere tekrar edilmiştir`);
}
